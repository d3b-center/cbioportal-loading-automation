...
def process_meta_data(meta_data, merged_file_dir, output_dir, canc_study_id):
...
def process_clinical_data(meta_data, clinical_file_dir, output_dir, canc_study_id):
    for dtype in meta_data["dtypes"]:
...
def write_case_list(case_key, attr_dict, sample_list, case_dir):
    ...
    case_file.write("cancer_study_identifier: " + study_id + "\n")
    key_list = list(attr_dict)
    case_file.write(
        key_list[0] + ": " + study_id + "_" + attr_dict[key_list[0]] + "\n"
    )
...
parser.add_argument(
    "-merged_file_dir",
    help="merged mafs, cnvs, rsems, fusion",
)
parser.add_argument(
    "-clinical_file_dir",
    help="clinical sample file and clinical patient file",
)
parser.add_argument(
    "-o",
    dest="output",
...
with open(args.data_config) as f:
    data_config_data = json.load(f)
with open(args.study_config) as f:
    study_data = json.load(f)

out_dir = args.output  +'/'

study_id = study_data["cancer_study_identifier"]
out_dir = args.output  +'/'  + study_id +'/'
case_lists = out_dir +'case_lists'
os.makedirs(case_lists)
process_meta_study(study_data,out_dir)
...
for key in data_keys:
    process_meta_data(config_data[key], args.merged_file_dir, out_dir, study_id)

process_clinical_data(config_data["data_sheets"], args.clinical_file_dir, out_dir, study_id)
create_case_lists(data_keys, out_dir)


# Run cbioportal data validator
errs = args.output + '/validator.errs'
err_out = args.output + '/validator.out'
validator_dir = 'cbioportal/core/src/main/scripts/importer/validateData.py'
validate = (
    validator_dir + " -s " + out_dir + " -n -v 2>" + errs + '> '+err_out
