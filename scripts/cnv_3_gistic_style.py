...
output = args.output
merged_cnv_dir =  args.output + '/merged_cnvs/'

fname_list = [f for f in listdir(merged_cnv_dir) if fnmatch(f, "*predicted_cnv.txt") if isfile(join(merged_cnv_dir, f))]
...
for fname in fname_list:
    cbio_dx = fname.split(".")[0]
    data = pd.read_csv(join(merged_cnv_dir, fname), sep="\t")
    ...
    for bs_id in bs_cbio_dict:
        mt_adjust_cn(bs_id)
    new_fname = cbio_dx = (
        merged_cnv_dir + "/" + cbio_dx + ".discrete_cnvs.txt"
    )
