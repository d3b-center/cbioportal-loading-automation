...
try:
    if os.path.isdir(config_data["data_sheets"]["dir"]):
        ...
        study_id = study_data["study"]["cancer_study_identifier"]
        cur_dir = out_dir + study_id + "/"
        try:
            os.mkdir(cur_dir)
        except:
            sys.stderr.write(cur_dir + " already exists.\n")
        sys.stderr.write("Creating meta study file for " + study_id + "\n")
        canc_study_id = process_meta_study(study_data["study"],cur_dir)
        data_keys = {
            "merged_mafs": 0,
            "merged_cnvs": 0,
            "merged_rsem": 0,
        ...
        for key in data_keys:
            if key in config_data and config_data[key]["dir"] != "":
                data_keys[key] = 1
                process_meta_data(config_data[key], cur_dir, study_id)
        ...
        process_clinical_data(config_data["data_sheets"], cur_dir, study_id)
